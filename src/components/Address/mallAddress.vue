<template>
  <div class="ovh">
    ...
    <el-table :data="list" style="width: 100%;margin-top:30px;" border>
      ...
      <!--地址类型'收货地址','发票地址' -->
      <el-table-column align="center" label="地址类型" width="80">
        <template slot-scope="scope">
          {{ scope.row.a_type | addressFilter }}
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :close-on-click-modal="false" width="60%" :append-to-body="true">
      <el-form ref="rules" :rules="rules" :model="temp" label-position="right" label-width="110px">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="地址" prop="province_city_district">
              <el-cascader v-model="temp.province_city_district" :props="props" @change="handleChange" style="width:100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="地址类型" prop="a_type">
              <el-radio-group v-model="temp.a_type">
                <el-radio :label="0">
                  收货地址
                </el-radio>
                <el-radio :label="1">
...
import { mallUsersAddresses, createAddress, updateAddress } from '@/api/mall'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        province_city_district: [{ required: true, message: '地址不能为空', trigger: 'blur' }],
  ...
  methods: {
    getList() {
      ...
      mallUsersAddresses(that.listQuery).then(response => {
