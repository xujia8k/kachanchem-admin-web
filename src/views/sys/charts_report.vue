<template>
  <div class="app-container">
    ...
    <el-table :data="list" style="width: 100%;margin-top:20px;" border fit highlight-current-row stripe class="cp">
      ...
      <el-table-column align="center" label="图表类型">
        <template slot-scope="scope">
          {{ scope.row.chart_type | chartTypeFilter}}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="修改日期" width="100">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="80%" :close-on-click-modal="false">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="110px">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="数据源设置" prop="chart_data">
              <el-input v-model="temp.chart_data" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="图表类型" prop="chart_type">
              <!-- <el-input v-model="temp.chart_type" /> -->
              <el-select v-model.trim="temp.chart_type" placeholder="请选择图表类型" clearable style="width: 100%;" class="filter-item" @change="handleFilter">
                <el-option v-for="item in chartTypeList" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="x轴tab" prop="tab_x_axis">
              <el-input v-model="temp.tab_x_axis" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="y轴tab" prop="tab_y_axis">
              <el-input v-model="temp.tab_y_axis" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="y轴数据" prop="y_axis">
              <el-input v-model="temp.y_axis" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="24">
            <el-button @click="addConditions" size="small" type="success" class="fr mb10">新增搜索条件</el-button>
          </el-col>
          <!-- 报表搜索条件 -->
...
import { chartsForms, addChartsForms, updateChartsForms, deleteChartsForms, chartsFormsRole, updateChartsFormsRole } from '@/api/sys'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        tab_x_axis: [{ required: true, message: 'x轴tab不能为空', trigger: 'blur' }],
        tab_y_axis: [{ required: true, message: 'y轴tab不能为空', trigger: 'blur' }],
        y_axis: [{ required: true, message: 'y轴数据不能为空', trigger: 'blur' }],
        chart_data: [{ required: true, message: '图表数据源定义不能为空', trigger: 'blur' }]
      },
      chartTypeList: [{
        "value": 0,
        "label": '自定义图表'
  ...
  methods: {
    getList() {
      ...
      chartsForms(this.listQuery).then(response => {
    ...
    handleDelete(row) {
      ...
      deleteChartsForms(tem).then(response => {
        if (response.code == 0) {
          this.$notify({
            title: '提示信息',
            message: '删除成功！',
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addChartsForms(this.temp).then(response => {
            if (response.code == 0) {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateChartsForms(tempData).then(response => {
            if (response.code == 0) {
              this.$notify({
                title: '提示信息',
                message: '更新成功！',
